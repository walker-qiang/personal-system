...
- personal-agent Python 定向测试：35/35 通过；全量测试 876 passed, 11 skipped。
...
- durable 研究卡写回与 API 回读：五粮液、腾讯各保存一张 schema v2 研究卡，AssetStore commit 和 API 回读均通过。

## 四、当前未完成的门槛

真实 Deep Research 内容质量门槛，以及两张代表性研究卡的 durable 写回和 API 回读均已通过。
历史研究卡清理已完成；当前尚未完成的是在 macOS App 中完成真实可读验收。
...
| 真实 Deep Research 内容质量 | 通过 | 五粮液、腾讯真实请求均完成一次修复并通过 |
| API 实际写回与回读 | 通过 | 五粮液、腾讯研究卡已通过 AssetStore 保存并可由 API 回读 |
| 自动化 personal-assets 闭环 | 未启动 | 需要先完成写回和 macOS App 验收 |

此前确认的 3 张历史 schema v2 研究卡已通过独立 commit 清理；本轮新增的 2 张研究卡均为通过
质量闸门后的有效写回。目录中的设计文档和今天的研究卡均保留。
...
1. 使用已保存的五粮液和腾讯研究卡进行 macOS App 端到端验证。
2. 检查研究结果长数字、来源、风险和结论的可读性。
3. 解决当前 macOS App 进程能回读 API 但不创建可见窗口的问题。
4. 确认新生成的研究不需要历史数据重跑即可使用新展示逻辑。
