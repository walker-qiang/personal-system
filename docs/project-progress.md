...
| `personal-assets` | durable source of truth，保存 finance facts 和 broader personal assets。 | `stable` | 已完成 `obsidian-wiki -> personal-assets` 迁移收口；`财富/` 已成为最终物理目录；已有 finance 目录、schemas、targets、事实文件；8 张 schema v2 研究卡已按稳定标的 ID 迁入 `财富/投研/标的/<target_id>/research/` 并保留 legacy record ID；作为 `personal-os` cache rebuild 来源已跑通；Vault 轻量校验通过。 | `知识/` 仍可随日常使用继续补强；海螺水泥和格力电器标的目录暂缺 profile；历史归档文档中仍可能保留少量旧术语。 | 以 `personal-assets` 作为唯一主目录继续维护；按实际使用补齐标的 profile、强化 `知识/` 并清理历史文档。 |
| `personal-os` | V1 为投资快照、投研数据和个人助理工作台，以及统一产品/API 入口；通过 HTTP/SSE 使用独立 `personal-agent`。 | `usable` | 已完成 finance cache builder、研究数据层、官方事实对账、研究卡质量闸门、观察池/基金池操作、估值快照、收益分析与本地自动历史、收益历史固定到标的 Vault、稳定 research record ID、标的目录新写入与新旧双路径读取、统一 AssetStore sync/commit/push、受控 Memory/Skill Vault API、持久化 ReviewService、任务重试、macOS App 和 Agent proxy；自动投研在 App 关闭后仍由 launchd 后台服务运行。 | 自动投研仍需长期观察消息质量、失败重试和真实复查成本；legacy 平铺研究卡读取暂保留兼容。 | 运行并观察自动投研与标的目录读写稳定性，在资产管理模型稳定后补齐 `持仓核算`，再评估移除 legacy 读取。 |
