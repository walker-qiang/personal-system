...
| `personal-os` | V1 为投资快照、投研数据和个人助理工作台，以及统一产品/API 入口；通过 HTTP/SSE 使用独立 `personal-agent`。 | `usable` | 已完成 finance cache builder、研究数据层、官方事实对账、研究卡质量闸门、观察池/基金池操作、估值快照、收益分析与本地自动历史、稳定 research record ID、标的目录新写入与新旧双路径读取、统一 AssetStore sync/commit/push、受控 Memory/Skill Vault API、持久化 ReviewService、任务重试、Web fallback 和 Agent proxy；自动投研在 App 关闭后仍由 launchd 后台服务运行。 | 自动投研仍需长期观察消息质量、失败重试和真实复查成本；legacy 平铺研究卡读取暂保留兼容。 | 运行并观察自动投研与标的目录读写稳定性，补齐收益分析固定到 Vault，并在确认无旧路径依赖后移除 legacy 读取。 |
