...
| `personal-assets` | durable source of truth，保存 finance facts 和 broader personal assets。 | `stable` | 已完成 `obsidian-wiki -> personal-assets` 迁移收口；`13-财富/` 已成为最终物理目录；已有 finance 目录、schemas、targets、事实文件；历史研究卡标的目录迁移已在 `47cd12a` 完成，后续 A 股质量观察池重建提交 `87622d8` 清理了该批历史卡；当前新研究卡继续按稳定标的 ID 写入并保留 legacy 路径兼容读取；作为 `personal-os` cache rebuild 来源已跑通；Vault 轻量校验通过。 | `21-知识/` 仍可随日常使用继续补强；海螺水泥和格力电器标的目录暂缺 profile；历史归档文档中仍可能保留少量旧术语。 | 以 `personal-assets` 作为唯一主目录继续维护；按实际使用补齐标的 profile、强化 `21-知识/` 并清理历史文档。 |
| `personal-os` | V1 为投资快照、投研数据和个人助理工作台，以及统一产品/API 入口；通过 HTTP/SSE 使用独立 `personal-agent`。 | `usable` | 已完成 finance cache builder、研究数据层、官方事实对账、研究卡质量闸门、观察池/基金池操作、估值快照、单资产持仓收益分析与本地自动历史、收益历史固定到标的 Vault、稳定 research record ID、标的目录新写入与新旧双路径读取、AssetStore 共享锁/路径校验/限定提交和按 operation 发布、受控 Memory/Skill Vault API、持久化 ReviewService、任务重试、macOS App 和 Agent proxy；自动投研在 App 关闭后仍由 launchd 后台服务运行。 | 自动投研仍需长期观察消息质量、失败重试和真实复查成本；legacy 平铺研究卡读取暂保留兼容；跨资产组合核算和更多成本口径尚未建设。 | 运行并观察自动投研与标的目录读写稳定性，评估跨资产组合核算和更多成本口径，再评估移除 legacy 读取。 |
...
| `personal-tools` | 长期复用工具仓；当前只维护 `weixin-clip` Chrome 扩展和 `workspace-git-delivery` Codex Skill。 | `active` | `582524e` 已移除旧 wiki-search、launchd 和通用脚本；`weixin-clip` 已到 v0.5.0 并通过静态/Manifest 校验；Git delivery Skill 已通过符号链接全局安装。 | 微信真实页面手工验收仍待完成；Skill 继续随实际交付流程校准。 | 完成 `weixin-clip` 真实页面验收，保持全局 Skill 符号链接与仓库源码一致。 |
...
| `personal-web` | finance UI 样式和交互参考；旧 admin / 管理页面行为参考。 | 必要 UI 经验已迁入 `personal-os/apps/mac`。 | ✅ **已归档** |
